...
export function landingTemplate(manifest: CustomManifest) {
      ...
      if (['text', 'number', 'password'].includes(elem.type)) {
        ...
        <div class="form-element">
          ...
          <div style="display: flex; gap: 10px;">
            ...
            ${key === 'febboxCookie' ? `<button type="button" id="testFebboxBtn" style="margin: 0; padding: 0 15px; font-size: 1.5vh;">Validate</button>` : ''}
          </div>
          ${key === 'febboxCookie' ? `<div id="febboxResult" style="margin-top: 5px; font-size: 1.5vh;"></div>` : ''}
        </div>
        `;
      ...
      if (testFebboxBtn) {
        testFebboxBtn.onclick = async (e) => {
          ...
          if (!cookie) {
            ...
            resultDiv.innerText = 'Please enter a cookie first';
            return;
          }
          testFebboxBtn.innerText = 'Testing...';
          try {
            ...
            resultDiv.style.color = '#ff4444';
            resultDiv.innerText = 'Network error occurred';
          } finally {
  ...
  <body>
    <div id="addon">
      ...
      <p>
        The source code can be found on <a href="https://github.com/devv5566/SwordWatch" target="_blank">GitHub</a>.
      </p>
      ...
      <p>
        Note: HTTP streams have limitations. For a better experience, I'd advise using a Debrid service and SwordWatch as fallback.
